...
var random = function(ι){return arguments.length===0? Math.random() : Tnum(ι)? random()*ι |0 : _u.sample(ι) }
var Time = function(ι){ var r = arguments.length===0? new Date() : ι instanceof Date? ι : new Date(Tnum(ι)? ι*1e3 : ι) ;r.toString = function(){return util.inspect(this) } ;return r }
Object.defineProperty(Date.prototype,'i',{get(){return +this / 1e3}})

//################################### prelude ###################################
var $ = ι=> [...document.querySelectorAll(ι)]
window.γ = window
γ.Buffer = require('buffer').Buffer
...
var t𐑱p_s𐑲z = [512,512]
var t𐑱p = Buffer.alloc(t𐑱p_s𐑲z.reduce((a,b)=>a*b))
var head_reg = 0 ;var head_at = 0

var update = I=>{ for( var i=0 ;i<I ;i++ ){
	// t ← tabl.find(ι=> ι[0]===head_reg && ι[1]===t𐑱p[head_at])
	var t = tabl[5*head_reg + t𐑱p[head_at]]
	var reg = t[2] ;var seen = t[3] ;var move = t[4]
	head_reg = reg
	t𐑱p[head_at] = seen
	var x = head_at % t𐑱p_s𐑲z[0]
	var y = head_at / t𐑱p_s𐑲z[0] |0
	switch( move ){ default: _interrobang_()
		;break ;case 0: x++ ;x += t𐑱p_s𐑲z[0] ;x %= t𐑱p_s𐑲z[0]
		;break ;case 1: x-- ;x += t𐑱p_s𐑲z[0] ;x %= t𐑱p_s𐑲z[0]
		;break ;case 2: y-- ;y += t𐑱p_s𐑲z[1] ;y %= t𐑱p_s𐑲z[1]
		;break ;case 3: y++ ;y += t𐑱p_s𐑲z[1] ;y %= t𐑱p_s𐑲z[1]
		}
	head_at = t𐑱p_s𐑲z[0] * y + x
	} }

window.addEventListener('load',()=>{
	γ.canvas = $('canvas')[0]
	canvas.width = t𐑱p_s𐑲z[0]
	canvas.height = t𐑱p_s𐑲z[1]
	canvas.ctx = canvas.getContext('2d')
	canvas.imgData = canvas.ctx.createImageData(...t𐑱p_s𐑲z)
...
var update_render = ()=>{
	...
	var t = canvas.imgData.data
	for( var i=0 ;i<t𐑱p.length ;i++ ){
		var c = color[t𐑱p[i]]
		t[4 * i + 0] = c[0]
		t[4 * i + 1] = c[1]
		t[4 * i + 2] = c[2]
