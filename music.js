...
speak= ([x,y],s)=>{
	...
	if( !s._ ){ s._= __sine_up() ;s._.head.start() ;s._.out.connect(AC.destination) }
	s._.gain.value= y
	s._.rate.value= rt
	// }
	return s }
...
drawTouch= _draw(X=>(xy,h,l,d=40,a=.1)=>{
	X.translate(...xy)
	X.beginPath()
...
dt22mgazu= (..._)=>x=> drawTouch([x[0]*screen.width*devicePixelRatio,x[1]*screen.height*devicePixelRatio],..._)
// equalize.map(dt22mgazu(1/5,20,40,.3))
// var x= numeric.linspace(0,1,2000) ;zip([ x,softsound_interpolate.at(x) ]).map(dt22mgazu(7/8,40,17,.1))
